...
public final class SereneSeasonsCompat {
    ...
    public static boolean hasSubSeasonChanged() {
    ...
    private static Object getSubSeason() {
        Object result = null;
        ...
        if (available) {
            try {
                ...
                if (getSubSeason == null) {
                    getSubSeason = seasonTime.getClass()
                        .getMethod("getSubSeason");
                }

                result = getSubSeason.invoke(seasonTime);
            } catch (IllegalAccessException | InvocationTargetException | NoSuchMethodException e) {
                BetterBiomeBlend.LOGGER.warn(
                    "Failed to read Serene Seasons state. Disabling Serene Seasons color cache compatibility.",
                    e);
                available = false;
            }
        }
    ...
    private static void findHooks() {
        ...
        if (Loader.isModLoaded(MOD_ID)) {
            try {
                ...
                BetterBiomeBlend.LOGGER
                    .warn("Serene Seasons is loaded, but Better Biome Blend could not find its season state hooks.", e);
                available = false;
            }
        } else {
            available = false;
